$(document).ready(function() {
    ...
    //text hidden toggle
    $('.text-hidden-link a').on('click', function() {
        if ($(this).hasClass('active')) {
            $(this).parent().parent().find('.text-hidden-wrap').slideUp(200);
            $(this).removeClass('active');
        } else {
            $(this).parent().parent().find('.text-hidden-wrap').slideDown(200);
            $(this).addClass('active');
        }
        return false;
    })
    //file input 
    $('.frm-field-attach .file-button').on('click', function () {
        $(this).parent().find('input').click();
        return false;
    })
    //section-tabs-slider
    if (!!$('.section-tabs-slider').offset()) {
    ...
    if (!!$('.section-tabs-slider').offset()) {
        $('.slider-outer-box .slider-nav-wrap .ico-arrow-prev').on('click', function () {
            $(this).parents('.slider-outer-box').find('.slider-wrap').find('.ico-arrow-prev').click();
            return false;
        })
        $('.slider-outer-box .slider-nav-wrap .ico-arrow-next').on('click', function () {
        ...
        $('.slider-outer-box .slider').on('afterChange', function (event, slick, currentSlide, nextSlide) {
            if ($(this).parents('.slider-outer-box').find('.slider-wrap').find('.ico-arrow-prev').hasClass('slick-disabled')) {
                $(this).parents('.slider-outer-box').find('.slider-nav-wrap').find('.ico-arrow-prev').addClass('slick-disabled');
            } else {
                $(this).parents('.slider-outer-box').find('.slider-nav-wrap').find('.ico-arrow-prev').removeClass('slick-disabled');
            }
            if ($(this).parents('.slider-outer-box').find('.slider-wrap').find('.ico-arrow-next').hasClass('slick-disabled')) {
                $(this).parents('.slider-outer-box').find('.slider-nav-wrap').find('.ico-arrow-next').addClass('slick-disabled');
            } else {
                $(this).parents('.slider-outer-box').find('.slider-nav-wrap').find('.ico-arrow-next').removeClass('slick-disabled');
            }
        })
    }
